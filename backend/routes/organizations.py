...
async def create_organization_manual(
    ...
    org_doc = {
        ...
        "forma_juridica": onrc_fields.get("forma_juridica", _detect_forma(denumire_extracted)),
        "nr_reg_com": onrc_fields.get("nr_reg_com", ""),
        "adresa": onrc_fields.get("adresa", ci_fields.get("adresa", "")),
        "cod_postal": onrc_fields.get("cod_postal", ""),
        "judet": onrc_fields.get("judet", ci_fields.get("judet", "")),
        "localitate": onrc_fields.get("localitate", ci_fields.get("localitate", "")),
        "stare": onrc_fields.get("stare", "ACTIVA"),
        "stare_detalii": "Extras automat din documente (GPT-5.2 Vision)",
        "data_infiintare": onrc_fields.get("data_infiintare", ""),
        "telefon": onrc_fields.get("telefon"),
        "tva": None,
        "tva_la_incasare": [],
        "capital_social": onrc_fields.get("capital_social"),
        "caen_principal": caen_principal,
        "caen_secundare": caen_secundare if isinstance(caen_secundare, list) else [],
        "administratori": administratori_onrc if administratori_onrc else ([{"nume": admin_name.strip(), "functie": "Administrator", "sursa": "CI OCR"}] if admin_name.strip() else []),
        "asociati": asociati_onrc if asociati_onrc else [],
        "nr_angajati": onrc_fields.get("nr_angajati"),
        "radiata": False,
